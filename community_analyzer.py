    ...
    def _compute_triangle_scores(self) -> Dict[Tuple[str, str, str], Tuple[str, float]]:
        ...
        for a, b, c in combinations(G.nodes, 3):
            if G.has_edge(a, b) and G.has_edge(b, c) and G.has_edge(c, a):
                ...
                print(
                    f"🔺 三角形: {a}, {b}, {c} → 構造: {struct}, スコア平均: {avg:.2f}"
                )
